  ...
  def singleFromInstance(instance: Instance, healthCheckResults: Seq[Health] = Nil): Option[EnrichedTask] =
    instance match {
      case instance @ Instance(instanceId, Some(agentInfo), _, Tasks(firstTask, _*), _, reservation, role) =>
        Some(EnrichedTask(instanceId.runSpecId, firstTask, agentInfo, healthCheckResults, Nil, reservation, role, cpus = instance.runSpec.resources.cpus, mem = instance.runSpec.resources.mem, resourceLimits = instance.runSpec.resourceLimits))
      case _ => None
    }
}